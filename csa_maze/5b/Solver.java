...
public class Solver {
    // consts
    public static final Color fwdPathColor = new Color(255, 0, 0);
    public static final Color bktPathColor = new Color(255, 255, 0);
    ...
    public boolean run(Cell cell, Side fromSide) {
        ...
        mc.drawPath(cell.getRow(), cell.getCol(), Side.Center, fwdPathColor);
        if (cell instanceof ExitCell) {
            return true;
        }

        ArrayList<Side> shuffledPaths = shuffle(cell.getPaths());
        for (Side path : shuffledPaths) {
            Cell nextCell = maze.getNeighbor(cell, path);
            if (!nextCell.getVisited()) {
                mc.drawPath(cell.getRow(), cell.getCol(), path, fwdPathColor);
                run(nextCell, getOpposite(path));
                if (maze.getExitCell().getVisited()) {
                    return true;
                }
                mc.drawPath(cell.getRow(), cell.getCol(), path, bktPathColor);
            }
        }
        mc.drawPath(cell.getRow(), cell.getCol(), fromSide, bktPathColor);
        mc.drawPath(cell.getRow(), cell.getCol(), Side.Center, bktPathColor);
        return false;
    }
